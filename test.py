...
if opq:
    R = np.fromfile('/root/gist_test/opq_matrix_100000_240_4_20000_20', dtype=np.float32).reshape(d, d)
    datas = [data @ R for data in datas]
    queries = [query @ R for query in queries]
...
cur.execute(sql)
sql = f'DROP INDEX IF EXISTS hnsw_idx;'
cur.execute(sql)
print('Begin Creating Index...')
sql = f'CREATE INDEX IF NOT EXISTS hnsw_idx ON psycopg2_items_{num_limit} USING hnsw (embedding vector_l2_ops) WITH (use_PQ={use_pq},pq_m={pq_m},nbits={nbits});'
cur.execute(sql)
...
ef_list_3 = list(range(100,620,20))
...
if opq == 1:
    with open(f"/home/postgres/output_{num_limit}_{use_pq}_{opq}_{pq_m}_{nbits}_{date_time}.txt", "w") as f:
        for a,b,c in output:
            f.write(f"{a} {b} {c}\n")
else:
    with open(f'/home/postgres/output_{num_limit}_{use_pq}_{date_time}.txt', "w") as f:
        for a,b,c in output:
            f.write(f"{a} {b} {c}\n")